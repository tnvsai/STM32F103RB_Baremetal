...
int main(void)
{
  ...
  if ((SHARED_MAGIC_FLAG != SHARED_MEMORY_VAL) || !BUTTON_STATUS)
  {
...
static void BL_JumpToUserApp(void)
{
  ...
  if (msp_value < 0x20000000 || msp_value > 0x20005000)
  {
    UART_Log(USART2, "Invalid MSP - BOOT TO APP DENIED\r\n");
    return;
  }
  ...
  if (sig_footer != NULL)
  {
    ...
    int signature_valid = Crypto_VerifySignature(calculated_hash, 32, sig_footer->signature, PUBLIC_KEY);
    ...
    else
    {
      ...
      UART_Log(USART2, "Signature INVALID - BOOT TO APP DENIED\r\n");
      return;
    }
  }
  else
  {
    ...
    if (crc_footer != NULL)
    {
      ...
      {
        UART_Log(USART2, "CRC OK - Booting to App...\r\n");
      }
      else
      {
...
static void BL_ProcessCommand(uint8_t cmd)
{
  ...
  case CMD_ERASE_APP:
    ...
    if (Flash_EraseAppRegion() == FLASH_OK)
    {
      ...
      UART_WriteChar(USART2, ACK);
    }
    else
    {
      UART_Log(USART2, "Result: FAIL\r\n");
      UART_WriteChar(USART2, NACK);
    }
    break;
